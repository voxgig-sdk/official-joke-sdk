...
//     (\`new OfficialJokeSDK(...)\` / \`OfficialJokeSDK.test(...)\` become
//     \`OfficialJokeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OfficialJokeSDK'
...
const TEST_SEED = {"entity":{"joke":{"test01":{"id":"test01"}},"type":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OfficialJokeSDK(...)\` and any
// \`OfficialJokeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OfficialJokeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OfficialJokeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
