...
(0, node_test_1.describe)('JokeDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.OfficialJokeSDK({
    ...
    (0, node_test_1.test)('direct-list-joke', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-joke', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["type01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.OFFICIAL_JOKE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.OfficialJokeSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['OFFICIAL_JOKE_TEST_JOKE_ENTID'];
    ...
    const client = new __1.OfficialJokeSDK({
        base: 'http://localhost:8080',
