...
[MCP](https://modelcontextprotocol.io) server exposing the OfficialJoke SDK as
two agent tools — `official-joke_list` and `official-joke_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/official-joke-mcp)
make build

# 2. Provide credentials via the environment
export OFFICIAL_JOKE_APIKEY=sk_live_xxx
...
```jsonc
// official-joke_list: first page of records
{ "entity": "joke" }
{ "entity": "joke", "query": { } }

// official-joke_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/official-joke-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user official-joke \
     -- "$PWD"/dist/*/official-joke-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `official-joke_list` and `official-joke_load` tools now appear
   in new sessions. Ask the agent to *"list joke using official-joke"*
   and it calls `official-joke_list` with `{"entity":"joke"}`.
...
```sh
export OFFICIAL_JOKE_APIKEY=sk_live_xxx            # API key
export OFFICIAL_JOKE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `official-joke_list` tool
...
### Call the `official-joke_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `official-joke_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `official-joke_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OFFICIAL_JOKE_APIKEY` | API key sent with every request. |
| `OFFICIAL_JOKE_BASE` | Optional override of the API base URL. |
