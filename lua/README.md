...
The Lua SDK for the OfficialJoke API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Joke(nil):list(nil, nil)
...
local result, err = client:OfficialJoke(nil):load(
  { id = "test01" }, nil
)
