...
This API provides a collection of jokes and allows users to retrieve random jokes or jokes by type.

Learn more about Official Joke API at [official-joke-api.appspot.com](https://official-joke-api.appspot.com).
...
This SDK exposes the API as a small set of **semantic entities** — Joke and Type — that you
...
const items = await client.Joke().list()
```
...
```go
client := sdk.Test()
result, err := client.Joke(nil).List(
    nil, nil,
)
```
...
$jokes = $client->Joke()->list();
print_r(array_map(fn($item) => $item->data_get(), $jokes));
