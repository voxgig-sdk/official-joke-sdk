# OfficialJoke PHP SDK



The PHP SDK for the OfficialJoke API — an entity-oriented client using PHP conventions.
...
try {
    ...
    foreach ($jokes as $record) {
        $item = $record->data_get();
        echo $item["id"] . " " . $item["punchline"] . "\n";
    }
} catch (\Throwable $err) {
...
print_r(array_map(fn($item) => $item->data_get(), $joke));
```
...
```php
require_once 'officialjoke_sdk.php';
$client = new OfficialJokeSDK($options);
```
...
```
php/
├── officialjoke_sdk.php          -- Main SDK class
...
The main class (`officialjoke_sdk.php`) exports the SDK class
