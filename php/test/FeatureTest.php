...
class FtCtrl extends OfficialJokeControl
{
...
class FtClient
{
    ...
    private OfficialJokeUtility $utility;

    public function __construct(OfficialJokeUtility $utility, array $options)
    {
    ...
    public function get_utility(): OfficialJokeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OfficialJokeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OfficialJokeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OfficialJokeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OfficialJokeConfig::make_feature($name);
    ...
    private static function buildUrl(OfficialJokeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OfficialJokeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OfficialJokeResponse($fetched);
        }
    ...
    private function fail(OfficialJokeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OfficialJokeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
