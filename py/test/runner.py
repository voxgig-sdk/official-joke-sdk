...
class OfficialJokeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OfficialJokeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OfficialJokeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OfficialJokeTestRunner.getenv("OFFICIAL_JOKE_TEST_LIVE")
        override = OfficialJokeTestRunner.getenv("OFFICIAL_JOKE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OfficialJokeTestRunner.getenv(key)
        ...
        explain = OfficialJokeTestRunner.getenv("OFFICIAL_JOKE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OFFICIAL_JOKE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OfficialJokeTestRunner._test_control is not None:
            return OfficialJokeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OfficialJokeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OfficialJokeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OfficialJokeTestRunner.load_test_control()
...
def load_env_local():
    OfficialJokeTestRunner.load_env_local()


def env_override(m):
    return OfficialJokeTestRunner.env_override(m)


def entity_data(v):
    return OfficialJokeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OfficialJokeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OfficialJokeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OfficialJokeTestRunner.load_test_control()


def live_client_options():
    return OfficialJokeTestRunner.live_client_options()


def live_delay_ms():
    return OfficialJokeTestRunner.live_delay_ms()
