...
The Python SDK for the OfficialJoke API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Joke(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.OfficialJoke(None).load(
    {"id": "test01"}, None
)
