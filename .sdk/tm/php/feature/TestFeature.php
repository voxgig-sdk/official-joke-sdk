...
// OfficialJoke SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class OfficialJokeTestFeature extends OfficialJokeBaseFeature
{
    ...
    public function init(OfficialJokeContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (OfficialJokeContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = OfficialJokeParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (OfficialJokeContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(OfficialJokeContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = OfficialJokeParam::call($ctx, $k);
