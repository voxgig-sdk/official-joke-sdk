...
// OfficialJoke SDK retry feature
...
class OfficialJokeRetryFeature extends OfficialJokeBaseFeature
{
    ...
    public function init(OfficialJokeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OfficialJokeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OfficialJokeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OfficialJokeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
