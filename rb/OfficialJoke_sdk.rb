...
require_relative 'OfficialJoke_types'


class OfficialJokeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OfficialJokeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OfficialJokeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OfficialJokeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OfficialJokeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OfficialJokeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OfficialJokeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OfficialJokeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => OfficialJokeError.new(
        "#{op}_allow",
        "OfficialJokeSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = OfficialJokeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OfficialJokeError.new(
        "graphql_error", "OfficialJokeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OfficialJokeSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
