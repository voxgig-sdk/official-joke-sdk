...
class TypeEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = OfficialJokeSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OFFICIAL_JOKE_TEST_TYPE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def type_basic_setup(extra)
  ...
  entid_env_raw = ENV["OFFICIAL_JOKE_TEST_TYPE_ENTID"]
  ...
  if env["OFFICIAL_JOKE_TEST_LIVE"] == "TRUE"
    ...
    client = OfficialJokeSDK.new(Helpers.to_map(merged_opts))
  end
