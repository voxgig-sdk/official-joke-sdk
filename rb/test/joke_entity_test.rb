...
class JokeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OFFICIAL_JOKE_TEST_JOKE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def joke_basic_setup(extra)
  ...
  idmap = Vs.transform(
    ["joke01", "joke02", "joke03", "random01", "random02", "random03"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
  ...
  entid_env_raw = ENV["OFFICIAL_JOKE_TEST_JOKE_ENTID"]
