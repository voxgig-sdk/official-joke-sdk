...
class TypeEntity
  ...
  def data_set(args)
    if args
      @_data = OfficialJokeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = OfficialJokeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Type>, Array] the matching Type items; raises OfficialJokeError on failure
