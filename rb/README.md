# OfficialJoke Ruby SDK



The Ruby SDK for the OfficialJoke API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Joke` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/official-joke-sdk/releases](https://github.com/voxgig-sdk/official-joke-sdk/releases)
...
```ruby
client = OfficialJokeSDK.test(testopts, sdkopts)
```
...
raise a `OfficialJokeError` (a `StandardError` subclass), so wrap
...
| --- | --- | --- |
| `id` | `Integer` | Unique identifier for the joke |
...
```
rb/
├── OfficialJoke_sdk.rb       -- Main SDK module
...
The main module (`OfficialJoke_sdk`) exports the SDK class
...
# joke.data_get now returns the joke data from the last list
