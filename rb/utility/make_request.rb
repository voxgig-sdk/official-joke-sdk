...
module OfficialJokeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OfficialJokeResponse.new({})
    ...
    if fetch_err
      ...
      response = OfficialJokeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OfficialJokeResponse.new(fetched)
    else
