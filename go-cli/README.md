...
boru-driven command-line client **and** interactive REPL for the OfficialJoke
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/official-joke-cli)
make build

# 2. See usage (words, entities, env vars)
./official-joke-cli --help

# 3. Provide credentials once, via the environment
export OFFICIAL_JOKE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./official-joke-cli list joke
./official-joke-cli load 1 joke            # {id:1} shorthand
./official-joke-cli load '{id:1}' joke       # explicit match map
./official-joke-cli list type

# 5. Override the API base URL for a single call
OFFICIAL_JOKE_BASE=https://api.example.com ./official-joke-cli list joke

# 6. No arguments -> interactive REPL
./official-joke-cli
official-joke> list joke
official-joke> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/official-joke-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/official-joke-cli list joke
   ```
...
```sh
./official-joke-cli load 1 joke          # scalar shorthand for {id:1}
./official-joke-cli load '{id:1}' joke     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `official-joke>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/official-joke-cli`. |
...
(`./official-joke-cli <expr>`) and interactively (the REPL), and expressions compose the
