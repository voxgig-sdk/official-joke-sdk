// Command OfficialJoke-cli is a CLI + REPL for the OfficialJoke SDK,
...
const prompt = "official-joke"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "joke type"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OFFICIAL_JOKE_APIKEY (api key), OFFICIAL_JOKE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OFFICIAL_JOKE_APIKEY carries the API key and
	// OFFICIAL_JOKE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OFFICIAL_JOKE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OFFICIAL_JOKE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOfficialJokeSDK(opts)
