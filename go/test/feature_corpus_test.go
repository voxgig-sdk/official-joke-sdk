...
//
// NewOfficialJokeSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.OfficialJokeSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewOfficialJokeSDK(opts)
}
...
func fcCandidates(client *sdk.OfficialJokeSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.OfficialJokeSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.OfficialJokeSDK, name string) any {
