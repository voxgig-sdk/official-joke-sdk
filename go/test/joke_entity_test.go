...
func TestJokeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OFFICIAL_JOKE_TEST_JOKE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func jokeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"joke01", "joke02", "joke03", "random01", "random02", "random03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("OFFICIAL_JOKE_TEST_JOKE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OFFICIAL_JOKE_TEST_JOKE_ENTID"])
	...
	if env["OFFICIAL_JOKE_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewOfficialJokeSDK(core.ToMapAny(mergedOpts))
	}
