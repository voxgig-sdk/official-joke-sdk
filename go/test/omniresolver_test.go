...
import (
	...
	omni "github.com/voxgig-sdk/official-joke-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OfficialJokeError); is {
				out := map[string]any{
					"name":    "OfficialJokeError",
...
func omniCtx(arg any, client *sdk.OfficialJokeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OfficialJokeSDK); is {
			client = live
