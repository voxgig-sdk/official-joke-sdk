# OfficialJoke Golang SDK Reference

Complete API reference for the OfficialJoke Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *OfficialJokeSDK`
...
#### `Joke(data map[string]any) OfficialJokeEntity`

Create a new `Joke` entity instance. Pass `nil` for no initial data.

#### `Type(data map[string]any) OfficialJokeEntity`
...
```go
result, err := client.Joke(nil).Load(map[string]any{"id": 1}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
