...
import (
	...
	vs "github.com/voxgig-sdk/official-joke-sdk/go/utility/struct"
)

type OfficialJokeSDK struct {
	Mode     string
...
func (sdk *OfficialJokeSDK) OptionsMap() map[string]any {
...
func (sdk *OfficialJokeSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *OfficialJokeSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *OfficialJokeSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OfficialJokeSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *OfficialJokeSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *OfficialJokeSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("OfficialJokeSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *OfficialJokeSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OfficialJokeSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("OfficialJokeSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *OfficialJokeSDK {
	...
	sdk := NewOfficialJokeSDK(sdkopts)
	sdk.Mode = "test"
